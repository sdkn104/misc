...
router = APIRouter(prefix="/read_docs", tags=["read_docs"])
...
@router.get("/")
def index():
    return "Hello, this is the read_docs router. Access /read_docs/read for the upload UI."


@router.get('/read', response_class=HTMLResponse)
async def index(request: Request):
    """アップロード UI を返す"""
    return templates.TemplateResponse(request, 'read_docs_index.html')


@router.post('/read/api/convert')
