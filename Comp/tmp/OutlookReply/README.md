...
| 重複防止 | HTTPリクエスト送信直後に重要度を「普通」に戻す。ウィンドウのクローズ検出と本文プレースホルダーの有無も併用 |
...
- 件名を変更するとウィンドウタイトルが変わり、同一ウィンドウが「閉じた」と誤判定される場合があります
  - **重要度「低」を設定する前**に件名を変更した場合：監視が中断され、改めて検出・監視が再開されます
  - **AIサーバーへのリクエスト送信後**（本文に「AIによる回答作成中...」が表示されている間）に件名を変更した場合：監視が中断され、AIレスポンスが返っても本文への挿入が行われません。「AIによる回答作成中...」が本文に残ったままになるため、手動で削除してください
