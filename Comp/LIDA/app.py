...
# logger.setLevel(logging.INFO) # DEBUGレベル以上のログを記録
#logger.propagate = False  # 親ロガー(デフォルトロガー)への伝播を防ぐ
...
async def visualize_data(req: VisualizeWebRequest) -> dict:
    ...
    try:
        ...
        logger.info(f"Received visualization request with goal") # === added log ===
