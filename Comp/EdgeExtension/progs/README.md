...
拡張IDを取得し（Ｅｄｇｅ拡張に手動インストールして確認）、以下を更新

@echo off
@REM Instalation Policy
REG ADD "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Edge\ExtensionSettings\idmemnoedlilhlncjjihlpcpajpfdaem" /v "installation_mode" /t REG_SZ /d "force_installed" /f

@REM Update Url
REG ADD "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Edge\ExtensionSettings\idmemnoedlilhlncjjihlpcpajpfdaem" /v "update_url" /t REG_SZ /d "https://localhost:8443/extension.crx" /f

@REM Show ToolBar Policy
REG ADD "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Edge\ExtensionSettings\idmemnoedlilhlncjjihlpcpajpfdaem" /v "toolbar_state" /t REG_SZ /d "default_shown" /f
echo Complete!!!
pause
